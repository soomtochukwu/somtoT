...
describe("Lock", function () {
    ...
    describe("Deployment", function () {
        ...
        it("Should receive and store the funds to lock", async function () {
            const { lock, lockedAmount } = await (0, network_helpers_1.loadFixture)(deployOneYearLockFixture);
            (0, chai_1.expect)(await hardhat_1.ethers.provider.getBalance(lock.target)).to.equal(lockedAmount);
        });
        it("Should fail if the unlockTime is not in the future", async function () {
            // We don't use the fixture here because we want a different deployment
            const latestTime = await network_helpers_1.time.latest();
            const Lock = await hardhat_1.ethers.getContractFactory("Lock");
    ...
    describe("Withdrawals", function () {
        ...
        describe("Events", function () {
            it("Should emit an event on withdrawals", async function () {
                const { lock, unlockTime, lockedAmount } = await (0, network_helpers_1.loadFixture)(deployOneYearLockFixture);
                await network_helpers_1.time.increaseTo(unlockTime);
        ...
        describe("Transfers", function () {
            it("Should transfer the funds to the owner", async function () {
                const { lock, unlockTime, lockedAmount, owner } = await (0, network_helpers_1.loadFixture)(deployOneYearLockFixture);
                await network_helpers_1.time.increaseTo(unlockTime);
